...
Here, we keep a dictionary that takes "channel" strings as keys and a list of
executables. Events can be "emitted" by calling the `emit_event` function,
passing in the data to emit and the channel name. This gets the list of callables
...
of a fully matured system of services in an animation production pipeline.
Undoubtedly they have some kind of message bus or event broker handling
communication between each of the services.
...
Here is an example to illustrate the differences between a traditional message
routing setup and Mycelia.

Let's suppose a customer tells a car insurance website that they have changed
addresses. The initial message data hits the broker who tells all subscribers
that the customer's address has updated. The Policy Generation Service has
subscribed to this event so that it can generate an updated rate for the
customer based on their new location.

Unfortunately the Policy Generation Service needs more data. It now needs to
know the accident volume of the new location. To get this, it emits an event
requesting this locational data that is picked up by the Geo Service.

The Geo Service then queries its database and sends the accident volume back to
the broker to send to the Policy Generation Service which then formulates the
new rate, and finally tells the broker to send the updated rate to the
customer.
...
The obvious fix is support for middleware.

This is what channel transformers do in Mycelia. Technically the transformer is
still sending a request on behalf of the first service to another with the
broker handling it between stops, just like in the original, but semantically
simpler. Here is a diagram comparison of the traditional vs hyphae routing
structures.

<p align="center">
<img src="https://i.imgur.com/qg4tDDE.png">
</p>

Because middleware is encapsulated in a channel, which are themselves encapsulated
in a route, the system allows for subscribers to specify where in the middlware
chain they want to receive a message.
